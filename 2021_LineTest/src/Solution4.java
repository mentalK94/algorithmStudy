...
public class Solution4 {
	...
	static class Node {
		...
		public Node(int id, String name, int pre, boolean leaf) {
			this.id = id;
			this.name = name;
			this.pre = pre;
			this.leaf = leaf;
	...
	static ArrayList<Node> leafNodeList;
	public String[] solution(String[] data, String word) {
		
		nodeList = new ArrayList<>();
        
		nodeList.add(new Node(0, "", -1, false)); // 최상단 루트노드 추가
		
		for(int i=0; i<data.length; ++i) {
			StringTokenizer st = new StringTokenizer(data[i]);
			int id = Integer.parseInt(st.nextToken());
			String name = st.nextToken();
			int preId = Integer.parseInt(st.nextToken());
			
			nodeList.add(new Node(id, name, preId, true));
			nodeList.get(preId).leaf = false;
		}
		...
		for(Node n : nodeList) { 
			if(n.leaf) {
				leafNodeList.add(new Node(n.id, n.name, n.pre, n.leaf));	
			}
		}
		
		// data 입력으로 들어온 값을 leafNodeList에서 비교
		int searchIdx = 0;		
		int first = 0;
		...
		for(int i=0; i<leafNodeList.size(); ++i) {
			// 1. 완전히 일치하는 지 확인
			if(leafNodeList.get(i).equals(word)) {
				searchIdx = leafNodeList.get(i).id;
				break;
			}
			
			// 2. word가 포함되어 있는지 확인
			for(int j=0; j<leafNodeList.get(i).name.length()-word.length(); ++j) {
				if(leafNodeList.get(i).name.substring(j, j+word.length()).equals(word)) {
					temp.add(j);
					j += (word.length()-1);
				}
			}
			
			if(loc.size()<temp.size()) {
				
			}
		}
        
		String[] answer = new String[1];
        answer[0] = "Your search for"+ word +" didn't return any results";
        return answer;
    }
}
